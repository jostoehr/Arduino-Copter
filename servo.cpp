...
{
	fid_servo = pca9685Setup(PIN_BASE, 0x40, HERTZ);
	if (fid_servo < 0)
	{
...
void Servo::update(float throttle, float PIDoutput[DIM])
{
	...
	for (int i = 0; i < 4; i++) {
		servoval[i] = (((410 - 234)*(servoval[i] - 1000)) / (2500 - 1000)) + 234;
	}
	setServo();
}
...
void Servo::setServo()
{
	if (Is_open_blaster()) {
		for (int i = 0; i < 4; i++) {
			pwmWrite(PIN_BASE + m_servoId[i], servoval[i]);
